...
describe("link testing", () => {
  ...
  it("internal and external classes are found in DOM", async () => {
    ...
    for (const l of internalLinks) {expect(l.classList.value, `class list should have had "internal-link": ${l.classList.value}`).toContain("internal-link");};
    ...
    for (const l of externalLinks) {expect(l.classList.value, `class list should have had "external-link": ${l.classList.value}`).toContain("external-link");};
...
expect(t.getAttribute("href"), 'the "href" should be empty in favor of a "to" prop').toBeNull()
    ...
    for (const t of router) {expect(t.getAttribute("to"), 'the "to" property should be set for <router-link>').toBeTypeOf("string");}
  });
...
expect(t.getAttribute("_base"), "the _base attribute is set during the build process to ensure a relative path but should be removed in final rendering").toBeNull()
  ...
  it("router links which are relative are resolved to a valid route", async () => {
    ...
    for (const t of router)  {expect(t.getAttribute("to").startsWith("test/fixtures"));}
  });
  ...
  it('router links add in "base" when present', async () => {
    ...
    for (const t of router)  {expect(t.getAttribute("to").startsWith("foobar/test/fixtures"));}
  });
});
